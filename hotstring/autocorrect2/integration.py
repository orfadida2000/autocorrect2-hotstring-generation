...
def has_custom_hotstring_include(
    ...
    *,
    ...
    """Return whether a custom file is included in AutoCorrect2's autocorrection context.

    Args:
        custom_file_path:
            Custom file to look for. A relative path is interpreted relative to
            the AutoCorrect2 project directory.
        project_dir:
            AutoCorrect2 project directory.

    Returns:
        `True` when exactly one include of the custom file exists inside the
    ...
    Raises:
        ...
        ValueError:
            If `custom_file_path` refers to one of the AutoCorrect2 files this
            integration logic itself relies on.
        AutoCorrect2IntegrationError:
            If the expected AutoCorrect2 `#HotIf` structure is missing,
            duplicated, malformed, or the custom include exists in an invalid
            or ambiguous location.
        OSError:
    ...
    return _inspect_include_state(custom_file_path, project_dir).custom_include_index is not None
...
def add_custom_hotstring_include(
    ...
    *,
    ...
    Raises:
        ...
        ValueError:
            If `custom_file_path` refers to one of the AutoCorrect2 files this
            integration logic itself relies on or cannot be represented safely
            in an AutoHotkey include directive.
        AutoCorrect2IntegrationError:
            If the expected AutoCorrect2 structure or current include state is
            inconsistent or ambiguous.
        OSError:
            If AutoCorrect2's main source file cannot be read or written.
    """
    ...
    comment = "; Library of custom hotstrings for auto-correction generated by the python package 'autocorrect2-hotstring-generation'."
...
def remove_custom_hotstring_include(
    ...
    *,
    ...
    Args:
        custom_file_path:
            Custom file whose include should be removed. A relative path is
            interpreted relative to the AutoCorrect2 project directory.
        project_dir:
            AutoCorrect2 project directory.
...
def _find_autocorrection_hotif(lines: list[str], source_path: Path) -> tuple[int, int]:
    """Return the start and end line indices of the autocorrection `#HotIf` block."""
    starts = [
        index
...
def _find_include_indices(
    ...
    *,
    ...
    for index, line in enumerate(lines):
        include_path = _parse_include_path(line, source_dir)
        if include_path is not None and _same_path(include_path, target_path):
...
def _parse_include_path(line: str, source_dir: Path) -> Path | None:
    ...
    # AutoCorrect2 is a Windows project, but normalizing both separators keeps
    # this parser deterministic in cross-platform tests as well.
